---
title: "differential_analysis"
format: html
---

# Setup

## Load packages

```{r Loading packages, warning=FALSE}
library(tidyverse) # v.2.0.0
library(embed) # v.1.1.3
library(umap) # v0.2.10.0
library(ggrepel) # v.0.9.4
library(readxl) # v.1.4.3
library(openxlsx) # v.4.2.7
library(HDAnalyzeR) # v.1.0.0
library(ggridges) # v.0.5.6
library(patchwork) # v.1.3.0
library(ComplexUpset) # v.1.3.3
library(doParallel) # v.1.0.17
library(viridis) # v.0.6.5
library(fmsb) # v.0.7.6
library(RColorBrewer) # v.1.1-3
library(writexl) # v.1.5.2
library(eulerr) # v.7.0.2
```

```{r}
select <- dplyr::select
filter <- dplyr::filter
mutate <- dplyr::mutate
arrange <- dplyr::arrange
rename <- dplyr::rename
```

## Source scrips

```{r Loading scripts}
source("../scripts/functions/functions_disease_analyses.R")
source("../scripts/functions/custom_plots.R")
```

## Read data

```{r Loading data, warning=FALSE}
# Read data
data_long <- read_csv("../data/data_long.csv")
data_wide <- read_csv("../data/data_wide.csv")
metadata <- read_csv("../data/metadata.csv")
olink_data_long <- read_csv("../data/olink_data_long.csv")
```

```{r Setting levels, include=FALSE}
# Setting levels
levels_pan_cancer <- c("AML", "CLL", "DLBCL", "Myeloma", "Colorectal", "Lung", "Glioma", "Breast", "Cervical", "Endometrial", "Ovarian", "Prostate", "HCC", "Pancreatic")
levels_all <- c(levels_pan_cancer, "Healthy")
```

```{r Setting palettes, include=FALSE}
# Defining sex-specific colors
pal_sex <- c("#30D5C8", "lightpink")
pal_sex <- setNames(pal_sex, c("Male", "Female"))
  
# Defining disease-specific colors
pal_all <- c("#8D9FCA", "#E8A29A", "#CDB18A", 
                 "#2271B5", "#B89B74", "#9E0142", 
                 "#B195AE", "#FFD321", "#D1CBE5", 
                 "#ADC74F", "#6D6D6D", "#66C2A5", 
                 "#603479", "#96C08E", "#E7662B")
pal_all <- setNames(pal_all, c("AML", "Breast", "Healthy", 
                                      "CLL", "Colorectal", "Cervical", 
                                      "Endometrial", "Glioma", "HCC", 
                                      "Lung", "DLBCL", "Myeloma", 
                                      "Ovarian", "Pancreatic", "Prostate"))


# Defining hematological cancer-specific colors
pal_hm <- c("#8D9FCA", "grey", "grey",
                   "#2271B5", "grey", "grey",
                   "grey", "grey", "grey",
                   "grey", "#6D6D6D", "#66C2A5", 
                   "grey", "grey", "grey")
pal_hm <- setNames(pal_hm, c("AML", "Breast", "Healthy", 
                                           "CLL", "Colorectal", "Cervical", 
                                           "Endometrial", "Glioma", "HCC", 
                                           "Lung", "DLBCL", "Myeloma", 
                                           "Ovarian", "Pancreatic", "Prostate"))

# Defining AML-healthy specific colors
pal_aml_healthy <- c("#CDB18A","#8D9FCA", "grey", 
                   "grey", "grey", "grey",
                   "grey", "grey", "grey",
                   "grey", "grey", "grey", 
                   "grey", "grey", "grey")
pal_aml_healthy <- setNames(pal_aml_healthy, c("Healthy", "AML", "CLL", 
                                           "DLBCL", "Myeloma", "Breast", 
                                           "Cervical", "Colorectal",
                                           "Endometrial", "Glioma", "HCC", "Lung", 
                                           "Ovarian", "Pancreatic", "Prostate"))
  
# Define colors for differential expression
pal_de <- c("#D3D3D3", "#FF7176", "#92C9DA")
pal_de <- setNames(pal_de, c("not significant", "significant up", "significant down"))
```

Overview of the Olink data and metadata:

```{r Head olink_data_long}
head(data_long)
```

```{r Head metadata}
head(metadata)
```

# Figure 2: Differential protein analysis

## Comparing AML patients to different controls

### A: Healthy

```{r}
de_aml_healthy <- hd_de_limma(dat = data_wide, metadata = metadata, variable = "Disease", case = "AML", control = "Healthy", correct = c("Age", "Sex"))
```

Differential expressed proteins in AML - Healthy controls.

```{r Count AML vs healthy}
# Upregulated proteins
de_aml_healthy_sig_up <- de_aml_healthy$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant up") %>%
  nrow()

cat("Number of significant upregulated proteins in AML - Healthy:", de_aml_healthy_sig_up)

# Downregulated proteins
de_aml_healthy_sig_down <- de_aml_healthy$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant down") %>%
  nrow()

cat("\nNumber of significant downregulated proteins in AML - Healthy:", de_aml_healthy_sig_down)
```

```{r DE AML vs healthy volcano, fig.width=10, fig.height=10}
# Plot volcano plot for the results
volcano_aml_healthy <- plot_volcano(de_aml_healthy$de_res |> rename(Assay = Feature) |>
                                      mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                                      adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                                      T ~ "not significant"))) +
  guides(fill = FALSE)
volcano_aml_healthy

ggsave("../plots/volcano/Volcano plot AML vs Healthy.pdf", width = 10, height = 10)
```

### B: Pan-cancer

```{r DE AML vs pan-cancer limma}
# Run differential expression using limma
de_aml_cancer <- hd_de_limma(dat = data_wide, metadata = metadata, variable = "Disease", case = "AML", control = c("Lung", "Ovarian", "Glioma", "Cervical", "Colorectal", "Prostate", "Breast", "Myeloma", "DLBCL", "CLL", "Endometrial", "HCC", "Pancreatic"), correct = c("Age", "Sex"))
```

Differential expressed proteins in AML - Pan-cancer controls.

```{r Count AML vs pan-cancer}
# Upregulated proteins
de_aml_cancer_sig_up <- de_aml_cancer$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant up") %>%
  nrow()

cat("Number of significant upregulated proteins in AML - Pan-cancer:", de_aml_cancer_sig_up)

# Downregulated proteins
de_aml_cancer_sig_down <- de_aml_cancer$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant down") %>%
  nrow()

cat("\nNumber of significant downregulated proteins in AML - Pan-cancer:", de_aml_cancer_sig_down)
```

```{r DE AML vs pan-cancer volcano plot, fig.width=10, fig.height=10}
# Plot volcano plot for the results
volcano_aml_cancer <-  plot_volcano(de_aml_cancer$de_res |> rename(Assay = Feature) |>
                                      mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                                      adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                                      T ~ "not significant"))) +
  guides(fill = FALSE)

volcano_aml_cancer

ggsave("../plots/volcano/Volcano plot AML vs pan-cancer.pdf", width = 10, height = 10)
```

```{r DE AML vs healthy and pan-cancer, fig.width=14, fig.height=10}
volcano_aml_healthy + volcano_aml_cancer
ggsave("../plots/volcano/Volcano plot AML vs healthy and pan-cancer.pdf", width = 14, height = 10)
```

## C: Comparing Healthy and Pan-cancer controls

Comparing the log2-fold change between the healthy and pan-cancer controls.

```{r Comparing DE healthy & pan-cancer controls, fig.width=10, fig.height=7}
# Comparing the Log2 fold changes between the healthy and pan-cancer controls
de_combined_healthy_cancer <- de_aml_healthy$de_res |> 
  select(Feature, logFC_healthy = logFC, adjpval_healthy = adj.P.Val) |> 
  left_join(de_aml_cancer$de_res |> 
              select(Feature, logFC_cancers = logFC, adjpval_cancers = adj.P.Val), by = "Feature")

# Annotate the top proteins
labels_scatter_healthy_cancer <- de_combined_healthy_cancer |> 
  top_n(n = 8, wt = abs(logFC_healthy + logFC_cancers)/2) 

de_aml_cancer_scatter <- de_combined_healthy_cancer |> 
  ggplot(aes(x = logFC_healthy, y = logFC_cancers, label = Feature, color = (logFC_healthy + logFC_cancers)/2)) +
  geom_point(size = 3, alpha = 0.8) +
  geom_smooth(method = "lm", formula = y ~ x, se = FALSE, color = "red", size = 1) +  # Add regression line
  geom_text_repel(data = labels_scatter_healthy_cancer, size = 4, 
                  box.padding = 0.8, 
                  segment.size = 0.5,
                  min.segment.length = 0,
                  max.overlaps = Inf,
                  arrow = arrow(length = unit(0.010, "npc")),
                  nudge_x = .15,
                  nudge_y = .5,
                  color = "grey50") +
  scale_color_gradient2(low = "blue", mid = "lightgray", high = "red", midpoint = 0) +
  annotate("text", x = max(de_combined_healthy_cancer$logFC_healthy), y = min(de_combined_healthy_cancer$logFC_cancers),
           label = sprintf("R = %.3f", cor(de_combined_healthy_cancer$logFC_healthy, de_combined_healthy_cancer$logFC_cancers)),
           hjust = 1, vjust = 0) +  # Add R value label
  scale_x_continuous(breaks = seq(floor(min(de_combined_healthy_cancer$logFC_healthy)), ceiling(max(de_combined_healthy_cancer$logFC_healthy)), by = 2.5)) +
  scale_y_continuous(breaks = seq(floor(min(de_combined_healthy_cancer$logFC_cancers)), ceiling(max(de_combined_healthy_cancer$logFC_cancers)), by = 2.5)) +
  theme_light()

de_aml_cancer_scatter

ggsave("../plots/scatter/Scatter plot AML vs Healthy and Cancer.pdf", width = 10, height = 7)
```

## D: Showing 4 boxplot examples across the controls

```{r DE example boxplots, fig.width=7, fig.height=10}
de_example_boxplots <- olink_data_long %>%
  select(DAid, Assay, NPX) %>%
  left_join(metadata, by = "DAid") %>%
  filter(Assay %in% c("FLT3", "FLT3LG", "EPO", "PGLYRP1")) %>%
  mutate(Assay = factor(Assay, levels = c("FLT3", "FLT3LG", "EPO", "PGLYRP1"))) %>%
  ggplot(aes(x = factor(Disease, levels = names(pal_aml_healthy)), y = NPX, color = Disease, fill = Disease)) + 
  geom_quasirandom(alpha = 0.8, show.legend = FALSE) + 
  geom_boxplot(color = "black", outlier.color = NA, alpha = 0.7) + 
  facet_wrap(~ Assay, ncol = 1, scales = "free_y") + 
  scale_color_manual(values = pal_aml_healthy) + 
  scale_fill_manual(values = pal_aml_healthy) +
  theme_light() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(x = "Cohort") +
  guides(fill = FALSE, color = FALSE)

de_example_boxplots

ggsave("../plots/boxplot/Boxplot AML vs Healthy and Cancer.pdf", width = 7, height = 10)
```

Number of samples per selected protein

```{r}
olink_data_long %>%
  select(DAid, Assay, NPX) %>%
  filter(Assay %in% c("FLT3", "FLT3LG", "EPO", "PGLYRP1")) |>
  group_by(Assay) |>
  summarize(n = n())
```

## Other controls

### S2C: Hematological malignancies

```{r DE AML vs hm limma}
# Run differential expression using limma
de_aml_hm <- hd_de_limma(dat = data_wide, metadata = metadata, variable = "Disease", case = "AML", control = c("CLL", "DLBCL", "Myeloma"), correct = c("Age", "Sex"))
```

Differential expressed proteins in AML - Hematological malignancies (CLL, DLBCL, Myeloma) as controls.

```{r Count AML vs hm}
# Upregulated proteins
de_aml_hm_sig_up <- de_aml_hm$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant up") %>%
  nrow()

cat("Number of significant upregulated proteins in AML - Hematological malignancies:", de_aml_hm_sig_up)

# Downregulated proteins
de_aml_hm_sig_down <- de_aml_hm$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant down") %>%
  nrow()

cat("\nNumber of significant downregulated proteins in AML - Hematological malignancies:", de_aml_hm_sig_down)
```

```{r DE AML vs hm volcano, fig.width=10, fig.height=7}
# Plot volcano plot for the results
volcano_aml_hm <- plot_volcano(de_aml_hm$de_res |> rename(Assay = Feature) |>
                                      mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                                      adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                                      T ~ "not significant"))) +
  guides(fill = FALSE)

volcano_aml_hm

ggsave("../plots/volcano/Volcano plot AML vs hm.pdf", width = 10, height = 7)
```

### S2D: CLL

```{r DE AML vs CLL limma}
# Run differential expression using limma
de_aml_cll <- hd_de_limma(dat = data_wide, metadata = metadata, variable = "Disease", case = "AML", control = "CLL", correct = c("Age", "Sex"))
```

Differential expressed proteins in AML - CLL controls.

```{r Count AML vs CLL}
# Upregulated proteins
de_aml_cll_sig_up <- de_aml_cll$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant up") %>%
  nrow()

cat("Number of significant upregulated proteins in AML - CLL:", de_aml_cll_sig_up)

# Downregulated proteins
de_aml_cll_sig_down <- de_aml_cll$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant down") %>%
  nrow()

cat("\nNumber of significant downregulated proteins in AML - CLL:", de_aml_cll_sig_down)
```

```{r DE AML vs CLL volcano, fig.width=10, fig.height=7}
# Plot volcano plot for the results
volcano_aml_cll <- plot_volcano(de_aml_cll$de_res |> rename(Assay = Feature) |>
                                      mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                                      adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                                      T ~ "not significant"))) +
  guides(fill = FALSE)

volcano_aml_cll 

ggsave("../plots/volcano/Volcano plot AML vs CLL.pdf", width = 10, height = 7)
```

### S2E: DLBCL

```{r DE AML vs DLBCL limma}
# Run differential expression using limma
de_aml_dlbcl <- hd_de_limma(dat = data_wide, metadata = metadata, variable = "Disease", case = "AML", control = "DLBCL", correct = c("Age", "Sex"))
```

Differential expressed proteins in AML - DLBCL controls.

```{r Count AML vs DLBCL}
# Upregulated proteins
de_aml_dlbcl_sig_up <- de_aml_dlbcl$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant up") %>%
  nrow()

cat("Number of significant upregulated proteins in AML - DLBCL:", de_aml_dlbcl_sig_up)

# Downregulated proteins
de_aml_cancer_dlbcl_down <- de_aml_dlbcl$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant down") %>%
  nrow()

cat("\nNumber of significant downregulated proteins in AML - DLBCL:", de_aml_cancer_dlbcl_down)
```

```{r DE AML vs DLBCL volcano, fig.width=10, fig.height=7}
# Plot volcano plot for the results
volcano_aml_dlbcl <- plot_volcano(de_aml_dlbcl$de_res |> rename(Assay = Feature) |>
                                      mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                                      adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                                      T ~ "not significant")))+
  guides(fill = FALSE)

volcano_aml_dlbcl

ggsave("../plots/volcano/Volcano plot AML vs DLBCL.pdf", width = 10, height = 7)
```

### S2F: Myeloma

```{r DE AML vs Myeloma limma}
# Run differential expression using limma
de_aml_myeloma <- hd_de_limma(dat = data_wide, metadata = metadata, variable = "Disease", case = "AML", control = "Myeloma", correct = c("Age", "Sex"))
```

Differential expressed proteins in AML - Myeloma controls.

```{r Count AML vs Myeloma}
# Upregulated proteins
de_aml_myeloma_sig_up <- de_aml_myeloma$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant up") %>%
  nrow()

cat("Number of significant upregulated proteins in AML - Myeloma:", de_aml_myeloma_sig_up)

# Downregulated proteins
de_aml_myeloma_sig_down <- de_aml_myeloma$de_res %>%
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  filter(sig == "significant down") %>%
  nrow()

cat("\nNumber of significant downregulated proteins in AML - Myeloma:", de_aml_myeloma_sig_down)
```

```{r DE AML vs Myeloma volcano, fig.width=10, fig.height=7}
# Plot volcano plot for the results
volcano_aml_myeloma <- plot_volcano(de_aml_myeloma$de_res |> rename(Assay = Feature) |>
                                      mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                                      adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                                      T ~ "not significant"))) +
  guides(fill = FALSE)

volcano_aml_myeloma

ggsave("../plots/volcano/Volcano plot AML vs Myeloma.pdf", width = 10, height = 7)
```

```{r Supplementary figures, fig.width=14, fig.height=20}
# Preparing figure S2
(volcano_aml_hm + volcano_aml_cll) / (volcano_aml_dlbcl + volcano_aml_myeloma)
ggsave("../plots/volcano/Volcano plots AML vs all hm controls.pdf", width = 14, height = 14)
```

```{r DE summary}
# Healthy control
protein_summary_aml_healthy <- de_aml_healthy$de_res |> 
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  select(Feature, logFC, adj.P.Val, sig) |>
  rename(logFC_healthy = logFC, adj.P.Val_healthy = adj.P.Val, sig_healthy = sig)

# Pan-cancer control
protein_summary_aml_cancer <- de_aml_cancer$de_res |> 
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  select(Feature, logFC, adj.P.Val, sig) |>
  rename(logFC_cancer = logFC, adj.P.Val_cancer = adj.P.Val, sig_cancer = sig)

# Hematological malignancies control
protein_summary_aml_hm <- de_aml_hm$de_res |> 
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  select(Feature, logFC, adj.P.Val, sig) |>
  rename(logFC_hm = logFC, adj.P.Val_hm = adj.P.Val, sig_hm = sig)

# CLL control
protein_summary_aml_cll <- de_aml_cll$de_res |> 
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  select(Feature, logFC, adj.P.Val, sig) |>
  rename(logFC_cll = logFC, adj.P.Val_cll = adj.P.Val, sig_cll = sig)

# DLBCL control
protein_summary_aml_dlbcl <- de_aml_dlbcl$de_res |> 
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  select(Feature, logFC, adj.P.Val, sig) |>
  rename(logFC_lymphoma = logFC, adj.P.Val_lymphoma = adj.P.Val, sig_lymphoma = sig)

# Myeloma control
protein_summary_aml_myeloma <- de_aml_myeloma$de_res |> 
  mutate(sig = case_when(adj.P.Val < 0.05 & logFC > 0 ~ "significant up",
                         adj.P.Val < 0.05 & logFC < 0 ~ "significant down",
                         T ~ "not significant")) %>%
  select(Feature, logFC, adj.P.Val, sig) |>
  rename(logFC_myeloma = logFC, adj.P.Val_myeloma = adj.P.Val, sig_myeloma = sig)

# Save the DE results to an Excel file
protein_summary <- protein_summary_aml_healthy |> 
  right_join(protein_summary_aml_cancer, by = "Feature") |>
  right_join(protein_summary_aml_hm, by = "Feature") |>
  right_join(protein_summary_aml_cll, by = "Feature") |>
  right_join(protein_summary_aml_dlbcl, by = "Feature") |>
  right_join(protein_summary_aml_myeloma, by = "Feature")
```

# Write to tables

```{r}
protein_summary |>
  write_xlsx("../tables/DA_proteins_AML_vs_controls.xlsx")
```

# Overlaps Between S2D-S2F

```{r}
protein_summary <- read_xlsx("../tables/DA_proteins_AML_vs_controls.xlsx")
```

```{r}
# Define sets with proper names
sets_up <- list(
  CLL = protein_summary %>%
    filter(sig_cll == "significant up") %>%
    pull(Feature),

  DLBCL = protein_summary %>%
    filter(sig_lymphoma == "significant up") %>%
    pull(Feature),

  Myeloma = protein_summary %>%
    filter(sig_myeloma == "significant up") %>%
    pull(Feature)
)

# Create Euler object
venn_plot <- euler(sets_up)

pdf("../plots/venn/Venn diagram upregulated proteins.pdf", width = 7, height = 7)

# Plot
plot(venn_plot,
     fills = list(fill = c("#2271B5", "#6D6D6D", "#66C2A5"), alpha = 0.5),
     edges = TRUE,
     labels = list(font = 2, cex = 1.5),        # <-- set names
     quantities = list(cex = 1.5)               # <-- numbers
)

dev.off()


sets_down <- list(
  CLL = protein_summary %>%
    filter(sig_cll == "significant down") %>%
    pull(Feature),

  DLBCL = protein_summary %>%
    filter(sig_lymphoma == "significant down") %>%
    pull(Feature),

  Myeloma = protein_summary %>%
    filter(sig_myeloma == "significant down") %>%
    pull(Feature)
)

venn_plot_down <- euler(sets_down)

pdf("../plots/venn/Venn diagram downregulated proteins.pdf", width = 7, height = 7)

plot(venn_plot_down,
     fills = list(fill = c("#2271B5", "#6D6D6D", "#66C2A5"), alpha = 0.5),
     edges = TRUE,
     labels = list(font = 2, cex = 1.5),
     quantities = list(cex = 1.5)
)

dev.off()
```



# SessionInfo

```{r}
sessionInfo()
```

End of file.
