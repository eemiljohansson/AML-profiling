...
umap_all <- umap_data$umap_res |>
  ...
  geom_label_repel(data = umap_data_means, aes(x = UMAP1_mean, y = UMAP2_mean, label = Disease), 
                   box.padding = 0.25, point.padding = 0.2, segment.color = NA,
...
```{r PCA}
# Default seed is 123
pca_data <- hd_pca(dat = data_wide)
```

```{r PCA means}
# Setting a seed
pca_data_means <- pca_data$pca_res |>
  left_join(metadata, by = "DAid") |>
  group_by(Disease) |>
  summarize(PC1_mean = mean(PC1), PC2_mean = mean(PC2))
```

```{r PCA plot all colors, fig.width=10, fig.height=7}
pca_all <- pca_data$pca_res |>
  right_join(metadata, by = "DAid") |>
  ggplot(aes(PC1, PC2, color = Disease, fill = Disease)) +
  geom_point(aes(color = Disease), alpha = 0.7, size = 2) +
  geom_label_repel(data = pca_data_means, aes(x = PC1_mean, y = PC2_mean, label = Disease), 
                   box.padding = 0.25, point.padding = 0.2, segment.color = NA,
                   size = 4, color = "black", alpha = 0.7, fontface = "bold") +
  theme_light() +
  scale_color_manual(values = pal_all) +
...
ggsave("../plots/PCA/PCA all.pdf", width = 10, height = 7)
```

### Highlighting hematological malignancies

```{r PCA plot HM colors, fig.width=10, fig.height=7}
pca_hm <- pca_data$pca_res |>
  right_join(metadata, by = "DAid") |>
  ggplot(aes(PC1, PC2, color = Disease, fill = Disease)) +
  geom_point(aes(color = Disease), alpha = 0.7, size = 2) +
  geom_label_repel(data = pca_data_means, aes(x = PC1_mean, y = PC2_mean, label = Disease), 
...
ggsave("../plots/PCA/PCA hematological malignancies.pdf", width = 10, height = 7)
```

```{r Supplementary PCA, fig.width=10, fig.height=14}
pca_all / pca_hm 

ggsave("../plots/PCA/PCA all and HM.pdf", width = 10, height = 14)
```

# Exporting information

```{r}
# Summarize number of samples per disease
metadata |>
  group_by(Disease) |>
  summarize(Number_of_samples = n()) |>
  # pivot wider
  pivot_wider(names_from = Disease, values_from = Number_of_samples) |>
  # Write to xlsx
  write.xlsx("../tables/number_of_samples_per_disease.xlsx")

# Summarize Age IQR1 median and IQR3 per disease
metadata |>
  filter(!is.na(Age)) |>
  group_by(Disease) |>
  summarize(Median_Age = median(Age),
            IQR1_Age = quantile(Age, 0.25),
            IQR3_Age = quantile(Age, 0.75)) |>
  # collape Median_Age (IQR1_Age - IQR3_Age)
  mutate(Age_Summary = paste0(Median_Age, " (", IQR1_Age, "-", IQR3_Age, ")")) |>
  # pivot wider
  select(Disease, Age_Summary) |>
  pivot_wider(names_from = Disease, values_from = Age_Summary) |>
  # Write to xlsx
  write.xlsx("../tables/age_summary_per_disease.xlsx")

# Summarize missing age in n (%)
metadata |>
  group_by(Disease) |>
  summarize(Missing_Age_n = sum(is.na(Age)),
            Total_n = n(),
            Missing_Age_percent = round((Missing_Age_n / Total_n) * 100, 2),
            Missing_Age_summary = paste0(Missing_Age_n, " (", Missing_Age_percent, ")")) |>
  # pivot wider
  select(Disease, Missing_Age_summary) |>
  pivot_wider(names_from = Disease, values_from = Missing_Age_summary) |>
  # Write to xlsx
  write.xlsx("../tables/missing_age_per_disease.xlsx")

# Count number of F in Sex in n and % of total count per disease
metadata |>
  group_by(Disease) |>
  summarise(
    n_F     = sum(Sex == "F", na.rm = TRUE),  # number of females
    total_n = n(),                            # total per disease
    perc_F  = 100 * n_F / total_n             # % F within each disease
  ) |>
  ungroup() |>
  # Write n_F (perc_F)
  mutate(summaryF = paste0(n_F, " (", round(perc_F, 2), ")")) |>
  select(Disease, summaryF) |>
  pivot_wider(names_from = Disease, values_from = summaryF) |>
  write.xlsx("../tables/female_sex_per_disease.xlsx")

# Missing sex
metadata |>
  group_by(Disease) |>
  summarise(
    n_missing_sex = sum(is.na(Sex)),   # number of missing Sex values
    total_n       = n(),               # total per Disease
    perc_missing  = 100 * n_missing_sex / total_n
  ) |>
  ungroup() |>
  # write n_missing_sex (perc_missing)
  mutate(summary = paste0(n_missing_sex, " (", round(perc_missing, 2), ")")) |>
  select(Disease, summary) |>
  pivot_wider(names_from = Disease, values_from = summary) |>
  write.xlsx("../tables/missing_sex_per_disease.xlsx")
```

```{r}
data_long_raw |>
  distinct(Assay, .keep_all = TRUE) |>
  select(Assay, OlinkID, UniProt, Panel) |>
  write.xlsx("../tables/assay_information.xlsx")
```
