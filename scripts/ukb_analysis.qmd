...
data_healthy |>
  ...
  write.csv("../data/UKB/UKB_Protein_ID_to_Assay.csv")

ukb_assay_converter <- read_csv("../data/UKB/UKB_Protein_ID_to_Assay.csv") |>
  select(Assay, Protein_ID)
```
...
# Save the plot
ggsave("../plots/violin/UKB_violin_plot_mean_DE.pdf", height = 10, width = 7)
```
...
# Save the plot
ggsave("../plots/violin/UKB_violin_plot_mean_ML.pdf", height = 10, width = 7)
```

```{r ukb lookup, fig.width=14, fig.height=10}
ukb_lookup_de + guides(color = F, fill = F) + ukb_lookup_ml

ggsave("../plots/combined/UKB_violin_plot_mean_DE_ML.pdf", height = 10, width = 14)
```
